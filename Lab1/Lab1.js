...
function readJsonFromFile(rates) {
    // ***YOUR CODE HERE***
      let data = fs.readFileSync(rates, 'utf-8'); // قراءة الملف كنص
      return JSON.parse(data); // تحويل النص إلى كائن JSON
  }
  //
                ...
                if (from !== to) {
                    ...
                    for (let from of symbols) {
                      for (let to of symbols) {
                          if (from !== to) {
                              let tag = `${from}-${to}`;
                              rates[tag] = usdMap[to] / usdMap[from]; // حساب التحويل بين العملات مباشرة
                          }
                      }
                  }
